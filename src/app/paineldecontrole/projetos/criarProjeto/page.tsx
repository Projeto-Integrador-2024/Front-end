...
export const CriarProjeto: React.FC<CreateProjectProps> = ({ className, ...props }) => {
    ...
    const [descricao, setDescricao] = useState<string>('');  // Estado para a descrição do projeto
    const [bolsa, setBolsa] = useState<string>('sem-bolsa');  // Estado para bolsa (opções: 'sem-bolsa', 'com-bolsa')
    const [tipo, setTipo] = useState<string>('extensao');  // Estado para tipo (opções: 'extensao', 'iniciacao-cientifica')
    ...
    const handleCreateProject = async () => {
        try {
            const response = await axios.post('http://127.0.0.1:5000/ADMIN/CREATE/VAGA', {
                nome: nome,
                descricao: descricao,
                bolsa: bolsa === 'com-bolsa',  // Converte para booleano (true ou false)
                tipo: tipo === 'extensao',  // Converte para booleano (true ou false)
                criador_id: "p0000002",  // ID do criador, pode ser adaptado conforme necessário
            }, {
                headers: {
